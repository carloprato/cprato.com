<?php
	...
	Class MidiModel extends BaseModel {
		...
		function insert($json, $file) {
			...
			$sql = 'INSERT INTO `midi`(`artist`, `title`, `edition`, `date`, `timestamp`, `url`, `downloads`, `beatport_id`, `length`, `release_date`, `bpm`, `key_scale`, `genre`, `label`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
			$q = $this->db->prepare($sql);						
			$req = $q->execute(array(
				$_POST['artist'],
				$_POST['title'], 
				...
				*/
				if (class_exists("OAuth")) {
					...
					*/
					...
					if ($json->results[0]->artists[0]->type == "artist") {
						$json->results[0]->artist = $json->results[0]->artists[1]->name;
					}
					return	$json;
				} else return "OAuth not installed.";
		}

		function upload($artist, $title, $edition) {

			$target_dir = $_SERVER['DOCUMENT_ROOT'] . "/midi/";
			$target_file = $target_dir . basename($artist . " - " . $title . " " . $edition . " (midi by Carlo Prato) (www.cprato.com).mid");
			$error = 0;
			$uploadOk = 1;
